(function(){
	...
	function formatDay(timestr){
		...
		each.call(ids['calenderDay'].getElementsByTagName("td"),function(o,i){
			...
			addEvent(o,"click",function(e){
				...
				var myinput=parent.getElementsByTagName("input")[0];
				myinput.value=ids['y'].innerHTML+"-"+ids['m'].innerHTML+"-"+ fillzero(o.innerHTML)
							+" ";
	...
	each.call([ids['yearL'],ids['yearR'],],function(o,i){
	...
	function getObj(className){
		...
		for(var i=0,len=o.length;i<len;i++){
			if(o[i].className.indexOf(className)>=0){
				oArr.push(o[i])
			}
